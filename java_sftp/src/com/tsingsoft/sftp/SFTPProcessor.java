...
public class SFTPProcessor {
	...
	public ChannelSftp getConnect(PropertyUtil proper, boolean isDownLoad) {
		...
		try {
			logger.info("sftp [ ftpHost = " + host + "  ftpPort = " + port
			        + "  ftpUserName = " + userName + "  ftpPassword = "
			        + password + " ]");
	 ...
	 *            输入流
	 */
	public boolean uploadFile(String uploadDir, String uploadFileName,
	        InputStream in) {
		...
		// }
		try {
			channel.cd(uploadDir);// 切换远程目录
			channel.put(in, uploadFileName);// 上传文件
			isSuccess = true;
		}
		catch (SftpException e) {
	 ...
	 * 文件下载
	 */
	public void downLoadFile(String sftpFileDir, String sftpFileName,
	        String baseFileName) {
		try {
			channel.cd(sftpFileDir);
			File file = new File(baseFileName);
			channel.get(sftpFileName, new FileOutputStream(file));
		}
		catch (Exception e) {
			e.printStackTrace();
		}
